...
def analyze_ble(file_name):
    ...
    print(f"总包数:ble: {total_packets},gpio:{len(gpio_result)}")
    ...
    if valid_scan_times:
        ...
        # 显示丢失包的详细信息
        if missing_packets > 0 and missing_packets <= 20:  # 限制显示数量
        ...
        print("没有有效的scan_time数据用于丢包统计")


    if gpio_result and ble_result:
        # 确保两个列表长度一致
        ...
        # 计算每个包的延迟
        ble_latencies = [ble_deltas[i] - gpio_deltas[i] for i in range(len(gpio_deltas))]
        print(ble_latencies)
        print(max(ble_latencies))
        print(min(ble_latencies))
...
if __name__ == "__main__":
    ...
    # print("\n生成scan_time详细报告...")
    # input_file = r"d:\Code\VScode\example\python\touch_optimize\captuer_data\bt_interval\KB04122-主板固定80Hz发送.txt"
